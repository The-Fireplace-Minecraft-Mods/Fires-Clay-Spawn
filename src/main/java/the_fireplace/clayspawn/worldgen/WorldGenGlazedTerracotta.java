...
public class WorldGenGlazedTerracotta extends WorldGenMinable {
	private final IBlockState oreBlock;
	private final int numberOfBlocks;
	private final Predicate<IBlockState> predicate = new StonePredicate();

	public WorldGenGlazedTerracotta(IBlockState state, int blockCount) {
							...
							for (int j2 = l; j2 <= k1; ++j2) {
								...
								if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D) {
									...
									if (state.getBlock().isReplaceableOreGen(state, worldIn, blockpos, this.predicate))
										worldIn.setBlockState(blockpos, this.oreBlock.getBlock().getStateFromMeta(rand.nextInt(EnumFacing.values().length)), 2);
								}
							}
					}
			}
