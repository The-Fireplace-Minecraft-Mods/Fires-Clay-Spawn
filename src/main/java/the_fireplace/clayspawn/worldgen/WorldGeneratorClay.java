	...
	private void generateSurface(World world, Random random, int BlockX, int BlockZ) {
		...
		if (ConfigValues.GENERATE && maxLayer - minLayer >= 0)
			for (int i = 0; i < getVeinChances(); i++) {
				...
				(new WorldGenMinable(Blocks.CLAY.getDefaultState(), maxVeinSize > minVeinSize ? random.nextInt(maxVeinSize - minVeinSize) + minVeinSize : minVeinSize)).generate(world, random, new BlockPos(Xcoord, Ycoord, Zcoord));
			}
		//Terracotta
		if (ConfigValues.HARDGENERATE && hardMaxLayer - hardMinLayer >= 0)
			for (int i = 0; i < getHardVeinChances(); i++) {
				...
				(new WorldGenMinable(hardState, hardMaxVeinSize > hardMinVeinSize ? random.nextInt(hardMaxVeinSize - hardMinVeinSize) + hardMinVeinSize : hardMinVeinSize)).generate(world, random, new BlockPos(Xcoord, Ycoord, Zcoord));
			}
		//Glazed Terracotta
		if (ConfigValues.GLAZEDGENERATE && glazedMaxLayer - glazedMinLayer >= 0)
			for (int i = 0; i < getGlazedVeinChances(); i++) {
				...
				(new WorldGenGlazedTerracotta(glazedState, glazedMaxVeinSize > glazedMinVeinSize ? random.nextInt(glazedMaxVeinSize - glazedMinVeinSize) + glazedMinVeinSize : glazedMinVeinSize)).generate(world, random, new BlockPos(Xcoord, Ycoord, Zcoord));
			}
	}
